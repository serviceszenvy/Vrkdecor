...
export function SiteFooter() {
  ...
  return (
    <footer className="border-line-soft bg-surface-inverse mt-4 border-t sm:mt-6">
      <Container width="wide">
        <div className="grid gap-10 py-14 sm:py-16 lg:grid-cols-12 lg:gap-8">
          <div className="flex flex-col gap-5 lg:col-span-4">
            <span className="inline-flex w-fit self-start rounded-2xl bg-white/95 px-3 py-2 shadow-panel">
              <Image
                src={logoSrc}
                alt={siteConfig.name}
                sizes="180px"
            ...
            <ul className="flex flex-col">
              {primaryNav.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-ink-muted hover:text-accent-300 group inline-flex min-h-9 items-center gap-1.5 text-sm transition-colors"
                  >
                    {item.label}
              ...
              {FOOTER_SERVICES.map((service) => (
                <li key={service.slug}>
                  <Link
                    ...
                    className="text-ink-muted hover:text-accent-300 inline-flex min-h-9 items-center text-sm transition-colors"
                  >
                    {service.name}
                  </Link>
                </li>
          ...
          <div className="flex flex-col gap-3 lg:col-span-3">
            ...
            <ul className="flex flex-col gap-3 text-sm">
              ...
              <li className="flex gap-3">
                ...
                <a
                  href={mailHref}
                  className="text-ink-muted hover:text-accent-300 break-all transition-colors"
                >
          ...
          <ul className="flex flex-wrap gap-4">
            {legalNav.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="text-ink-muted hover:text-accent-300 text-xs transition-colors"
                >
                  {item.label}
                </Link>
              </li>
